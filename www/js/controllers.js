...
.controller('diagnosemeCtrl', function($scope, $state, $firebaseArray,$ionicPopup, Utility) {
  var refurl = "https://diagnosediabetes.firebaseio.com/";
  var ref = new Firebase(refurl);
  var authData = ref.getAuth();
  if (authData) {
  ...
  $scope.submit = function(parameter1) {
    ...
    $scope.patientparameter.$add(res).then(function(ref) {
      $scope.report = true;
      $ionicPopup.alert({
        title:'Alert',
...
.controller('presultCtrl', function($scope, $state, $firebaseArray,$ionicPopup, Utility) {
